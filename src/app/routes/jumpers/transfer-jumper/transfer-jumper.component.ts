...
@Component({
  ...
  styleUrls: ['./transfer-jumper.component.scss', '../../../components/form/form.component.scss']
})
export class TransferJumperComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    axios.get(environment.API_URL + '/jumpers', {
      headers: { Authorization: 'Bearer ' + token }
    }).then((response) => {
      this.jumpers = response.data.value;
      this.jumpers.forEach(jumper => {
        jumper.name = jumper.firstName + ' ' + jumper.lastName + ' | ' + jumper.base.code
      });
  ...
  transferJumper = (event) => {
    let token = window.sessionStorage.getItem('token');
    axios.get(environment.API_URL + '/jumpers/transfer/?baseCode=' + this.baseCode + '&baseId=' + this.baseId + '&jumperId=' + this.selectedJumper.id, {
      headers: { Authorization: 'Bearer ' + token }
    }).then((response) => {
